...
box_info = BaseBox()
available_boxes = box_info.get_avalible_boxes()
box_info.log_info_boxes(available_boxes)

selected_box = str(input('\nNumber of box: '))
amount_boxes = int(input(f'\nAmount boxes (Max: {available_boxes[selected_box]["limit_amount"]}): '))

product_id = available_boxes[selected_box]['product_id']
